...
function AudioPlayer({ filePath, getSignedUrl }: { filePath: string; getSignedUrl: (path: string) => Promise<string | null> }) {
...
export function MediaTools({ practiceLogId, userId }: MediaToolsProps) {
  const {
    mediaItems,
    isLoading,
    isUploading,
        ...
        <div className="space-y-2">
          {mediaItems.map((item) => (
            <MediaItemCard
              ...
              getSignedUrl={getSignedAudioUrl}
            />
          ))}
        </div>
      )}
  ...
  return (
    <div className="border border-border rounded-md p-2 space-y-1">
      ...
      {item.media_type === "audio" && item.file_path && (
        <AudioPlayer filePath={item.file_path} getSignedUrl={getSignedUrl} />
      )}
