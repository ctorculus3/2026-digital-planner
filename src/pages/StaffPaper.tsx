...
function StaffLines() {
  return <div className="flex flex-col gap-4">
       {[...Array(5)].map((_, i) => <div key={i} className="h-px bg-foreground/80 w-full" />)}
     </div>;
}
  ...
  return <div className="min-h-screen bg-background flex flex-col">
       ...
       <div className="border-b border-border px-4 py-3 flex items-center justify-between bg-[#f7f3f3]">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant={isErasing ? "default" : "outline"} size="sm" onClick={toggleEraser} title={isErasing ? "Switch to pen (double-tap pencil)" : "Switch to eraser (double-tap pencil)"} className="flex items-center gap-1">
            {isErasing ? <Eraser className="h-4 w-4" /> : <Pencil className="h-4 w-4" />}
          </Button>
          <Button variant="default" size="sm" onClick={saveDrawing} disabled={isSaving || !hasUnsavedChanges} className="flex items-center gap-1">
            {isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
            Save
          </Button>
          <Button variant="ghost" size="icon" onClick={clearDrawing} title="Clear drawing">
       ...
       <div className="flex-1 p-6 md:p-8 overflow-auto">
        <div ref={containerRef} className="max-w-4xl mx-auto bg-card rounded-lg shadow-md p-6 md:p-8 relative">
          {/* Staves - 8 sets of 5 lines each */}
          <div className="flex flex-col gap-16 relative z-0">
            {[...Array(8)].map((_, staveIndex) => <StaffLines key={staveIndex} />)}
          </div>
          
          {/* Drawing Canvas Overlay */}
          <div className="absolute inset-0 z-10">
            <DrawingCanvas width={canvasSize.width} height={canvasSize.height} drawingData={drawingData} onDrawingChange={updateDrawing} isLoading={isLoading} isErasing={isErasing} onToggleEraser={toggleEraser} />
           </div>
         </div>
       </div>
     </div>;
